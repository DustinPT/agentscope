...
export function ChannelPage() {
        ...
        React.useEffect(() => {
                channelApi.listTypes().then(setTypes).catch(() => {});
        }, []);
        ...
        const typeOf = (channelType: string) =>
                types.find((item) => item.channel_type === channelType);

        const agentName = (agentId: string) =>
                agents.find((agent) => agent.id === agentId)?.data.name ?? agentId.slice(0, 8);

        const deleteName = (channel: ChannelRecord) =>
                channel.name?.trim() ||
                `${typeOf(channel.channel_type)?.display_name ?? channel.channel_type} · ${channel.id.slice(0, 8)}`;
