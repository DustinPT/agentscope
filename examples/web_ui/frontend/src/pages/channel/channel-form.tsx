...
function sessionSettings(value: ChannelFormValue) {
        return {
                session: {
                        chat_model_config: value.chatModelConfig as ChatModelConfig,
                        fallback_chat_model_config: value.fallbackChatModelConfig,
                        permission_mode: value.permissionMode,
                },
        };
}

export function toCreateRequest(value: ChannelFormValue): CreateChannelRequest {
        return {
                channel_type: value.channelType,
                name: value.name.trim() || null,
                credentials: value.credentials,
                platform_config: value.platformConfig,
                routing: { bindings: value.bindings },
                enabled: true,
                ...sessionSettings(value),
        };
}

export function toUpdateRequest(value: ChannelFormValue): UpdateChannelRequest {
        return {
                name: value.name.trim() || null,
                platform_config: value.platformConfig,
                routing: { bindings: value.bindings },
...
export function ChannelForm({ value, onChange, agents, channelTypes, mode }: Props) {
        ...
        const set = <K extends keyof ChannelFormValue>(key: K, nextValue: ChannelFormValue[K]) =>
                onChange({ ...value, [key]: nextValue });

        const typeSchema = React.useMemo(
                () => channelTypes.find((channelType) => channelType.channel_type === value.channelType),
        ...
        const selectedModelCard = React.useMemo(() => {
                ...
                for (const { models } of items) {
                        const card = models.find((model) => model.name === value.chatModelConfig!.model);
                        if (card) return card;
                }
                return null;
                ...
                <FieldGroup className="[&>[data-orientation=horizontal]>:last-child]:w-48">
                        {mode === 'edit' && (
                                <Field orientation="horizontal">
                                        ...
                                        <span className="text-sm">
                                                {typeSchema?.display_name ?? value.channelType}
                                        </span>
                                </Field>
                        )}
                        ...
                        <Field orientation="horizontal">
                                ...
                                <div className="flex w-full items-center gap-1">
                                        <LlmSelect
                                                ...
                                                onChange={(nextValue) => set('chatModelConfig', nextValue)}
                                        />
                                        <ModelParametersPopover
                                                ...
                                                onFallbackChange={(config) =>
                                                        set('fallbackChatModelConfig', config)
                                                }
                                        />
                                </div>
                        </Field>
                        ...
                        <Field orientation="horizontal">
                                ...
                                <PermissionModeSelect
                                        ...
                                        onChange={(nextValue) => set('permissionMode', nextValue)}
                                />
                        </Field>
                        ...
                        <Field>
                                ...
                                <BindingsEditor
                                        ...
                                        onChange={(bindings) => set('bindings', bindings)}
                                        agents={agents}
                                />
                        </Field>
...
export function isChannelFormValid(value: ChannelFormValue, mode: 'create' | 'edit'): boolean {
        if (!value.name.trim()) return false;
        if (!value.chatModelConfig) return false;
        if (value.bindings.length === 0) return false;
        if (value.bindings.some((binding) => !binding.agent_id)) return false;
        if (mode === 'create' && !value.channelType) return false;
        return true;
}
