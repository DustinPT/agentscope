...
async def _import_package_agent_assets(
    *,
    ...
    try:
        ...
        _ensure_unique_mcp_names([], staged_mcp_assets)
...
async def import_agent_package(
    ...
    try:
        ...
        for staged_agent in staged_package.agents:
            existing_by_id[staged_agent.id] = await storage.get_agent(
                user_id,
                staged_agent.id,
            )
