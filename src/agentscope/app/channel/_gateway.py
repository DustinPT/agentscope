...
from ._base import ChatKind, ChannelEvent, ChannelConfirmationResultEvent
...
class ChannelGateway:
    ...
    async def _ensure_session(
        ...
        initial_state = AgentState(
            conversation_kind=(
                chat_kind.value if chat_kind is not None else None
            ),
    ...
    @staticmethod
    def _event_chat_kind(event: ChannelEvent) -> ChatKind | None:
        """Infer chat audience shape from inbound event metadata."""
        chat_type = str(event.metadata.get("chat_type", "") or "").lower()
        return {
