...
class MiddlewareBase:  # pylint: disable=unused-argument
    ...
    **Onion Pattern Hooks** (with before/after logic):
    ...
    - `on_compress_context`: Intercepts context compression while
      preserving the project's display-event stream
    ...
    **Transformer Pattern Hook** (sequential pipeline):
    ...
    async def on_compress_context(
        ...
        Unlike upstream AgentScope, this project intentionally preserves the
        compression event stream so the frontend can visualize compression in
        real time. The hook still aligns with the upstream argument contract
        by forwarding optional ``instructions`` alongside ``context_config``.
